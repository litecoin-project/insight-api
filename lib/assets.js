...
AssetController.prototype._loadBestUtxos = function(body, valueToReach, includeAssets) {
  const address = body.address;

  return new Promise((resolve, reject) => {
    this._getUtxo(address, this.addressController.ccBuilderTransformUtxo)
      .then((utxos) => {
        let valueSum = 0;
        const minUtxos = [];
...
AssetController.prototype._loadMinimumUtxosCycle = function(body, valueToReach, type, includeAssets) {
  const buildFunc = this.ccBuildTypes[type];

  return new Promise((resolve, reject) => {
    const loadCycle = (valueNeeded) => {
      this._loadBestUtxos(body, valueNeeded, includeAssets)
        .then((utxos) => {
          body.utxos = utxos;
          const buildResult = buildFunc(body);
          const txn = buildResult.txb.tx;

          this._evaluateInputsValue(body, txn)
            .then((result) => {
              if (result.pass) {
                resolve(buildResult);
              } else {
...
AssetController.prototype._calculateMiningFee = function(numInputs, numOutputs) {
  const byteSize = (numInputs * 180) + (numOutputs * 34) + 10;
  return new Promise ((resolve, reject) => {
    this._fetchEstimateFee()
      .then((estimateFee) => {
        const fee = byteSize * (estimateFee / 1000);
...
AssetController.prototype._evaluateInputsValue = function(body, txn) {
  const inputs = txn.ins.length;
  const outputs = txn.outs.length;

  const calcMiningFee = this._calculateMiningFee(inputs, outputs);
  const valueCollected = this._calculateInputsValue(body.utxos);

  return new Promise((resolve, reject) => {
    calcMiningFee.then((miningFee) => {
      const valueNeeded = miningFee + body.amount;
      if (valueCollected > valueNeeded) {
        resolve({ pass: true, valueNeeded });
      } else {
        resolve({ pass: false, valueNeeded });
      }
    }).catch(reject);
  });
};
    ...
    this._evaluateInputsValue(body, txn)
      .then((result) => {
        if (result.pass) {
          ...
          this._loadMinimumUtxosCycle(body, result.valueNeeded, type, includeAssets)
            .then(res.jsonp)
            .catch((err) => {
              this.common.handleErrors(err, res);
            });
...
AssetController.prototype._fetchEstimateFee = function() {
