...
AssetController.prototype.buildIssueTransaction=function(req,res){
  ...
  if (!body.utxos){ 
      self._getUtxo(body.address, self.addressController.ccBuilderTransformUtxo,function (err, utxos) { 
        if (err){
         return self.common.handleErrors(err,res);
        } 
        body.utxos=utxos;
...
AssetController.prototype.buildSendTransaction=function(req,res){
  ...
  if (!body.utxos){  
      self._getUtxo(body.address,self.addressController.ccBuilderTransformUtxo, function (err, utxos) { 
        ...
        self._selectBestUtxos(utxos).then(function(utxos){  
               body.utxos=utxos;
...
AssetController.prototype._selectBestUtxos=function(utxos){
  ...
  if (utxos==undefined || utxos.length<=0){
    ...
    self.txController._transaction(select_max_utxo.txid).then(function(transaction){
      ...
      var utxos_matched=[];
      utxos_matched.push(select_max_utxo);
      deferred.resolve(utxos_matched);
    }).catch(function(err){
      deferred.reject(err);
    });
  }
...
AssetController.prototype._getUtxo=function(address,transform,callback){
  var self=this;
  ...
  self.addressController._utxo(address, transform, function (err, utxos) {
      if (utxos==undefined || utxos.length<=0){
             return callback({message:'The address does not have inputs'});
      } 
