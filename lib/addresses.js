...
AddressController.prototype.utxo = function(req, res) {
  var self = this;
  self._utxo(req.addr, function(err, utxos) {
    if(err) {
      return self.common.handleErrors(err, res);
    }  
    res.jsonp(utxos);
...
AddressController.prototype._utxo= function (address, callback) {
  var self = this;
  
  this.node.getAddressUnspentOutputs(address, {}, function(err, utxos) {
    if(err) {
        callback(err)
    ...
    return callback(null, utxos.map(self.transformUtxo.bind(self)));
  });
};
