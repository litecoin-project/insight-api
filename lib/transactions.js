...
TxController.prototype.transaction = function(req, res, next) {
  ...
  var txid = req.params.txid; 
   ...
   self._transaction(txid)
            .then(function(transformedTransaction){
...
TxController.prototype._transaction = function(txid) { 
  ...
  self.node.getDetailedTransaction(txid, function(err, transaction) { 
    ...
    self.transformTransaction(transaction, function(err, transformedTransaction) { 
      ...
      deferred.resolve(transformedTransaction); 
    });
...
TxController.prototype.checkAndStoreCCTransaction = function (txid, callback) {
  var self = this;
  this.getTransformTransaction(txid, function (err, transformedTx) {
    var ccData = self.ccParser.parseTransaction(transformedTx);
    if (ccData.length > 0) {
      self.storeCCTransacction(transformedTx, ccData, callback);
    } else {
      callback(null, false);
    }
  });
 ...
 */
TxController.prototype.storeCCTransacction = function (tx, ccData, callback) {
  var self = this;
